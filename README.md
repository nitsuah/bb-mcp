...
| `TRUST_PROXY_TLS` | Required (with `TLS_CERT_PATH`/`TLS_KEY_PATH` as the alternative) on non-loopback `HOST` | Set to `true` only when a trusted TLS-terminating reverse proxy (nginx, Traefik, Caddy, etc.) already sits in front of this port. **The server refuses to start on a non-loopback `HOST` unless this or `TLS_CERT_PATH`/`TLS_KEY_PATH` is set** — this server speaks plain HTTP on its own, and an on-path attacker could otherwise capture and replay the `MCP_API_KEY` bearer token. |
...
**Transport gate (`MCP_API_KEY`)** — the `/mcp` endpoint itself has no other credential check, so without `MCP_API_KEY` configured, anyone who can reach the port can call any tool asserting any `caller_identity`. The server fails closed on a non-loopback `HOST` (`127.0.0.1`/`localhost`/`::1` is the only host that may skip it): it refuses to start without a key, **and** it refuses to start without either `TLS_CERT_PATH`/`TLS_KEY_PATH` (serve HTTPS directly) or an explicit `TRUST_PROXY_TLS=true` (a TLS-terminating reverse proxy is already in front of it) — this server speaks plain HTTP on its own, and a bearer token sent that way can be captured and replayed by an on-path attacker. See [Configuration](#configuration).
